...
async def test_acquire_lock():
    ...
    with patch("crud.crud_instance.lock_acquire_db") as mock_db:
        mock_db.return_value = {
            "user": "44a842a7-6675-41fd-b1e1-291896a2f9fb",
            "status": "locked",
        }
        ...
        async with AsyncClient(
            ...
            assert response.status_code == 200
            assert ("user" in response.json()) and isinstance(
                response.json()["user"], str
            )
            assert ("locktime" in response.json()) and (
                (response.json()["locktime"] is None)
                or isinstance(response.json()["locktime"], float)
            )
            assert ("status" in response.json()) and isinstance(
                response.json()["status"], str
            )
